...
\begin{tikzpicture}[
  ...
  box/.style={rectangle, draw=gray!60, fill=blue!6, rounded corners=3pt,
              minimum width=3.0cm, minimum height=0.55cm,
              font=\scriptsize\sffamily, align=center},
  decision/.style={diamond, draw=gray!60, fill=orange!10, rounded corners=2pt,
  ...
  \node[box, below=of ingest]  (scan)    {Static Scanner\\(Regex / Semgrep)};
  \node[box, below=of scan]    (hook)    {Hook Detector\\(Consent Gap)};
  \node[box, below=of hook]    (chain)   {Chain Tracer\\(Import Graph)};
  \node[box, below=of chain]   (llm)     {LLM Auditor\\(opt-in)};
  \node[box, below=of llm]     (reg)     {Registry Adapter\\(opt-in)};
  \node[box, below=of reg]     (policy)  {Policy Engine};
  \node[decision, below=0.4cm of policy] (dec) {PASS?};
  \node[box, below=0.4cm of dec, fill=green!8]  (merkle)  {Merkle Tree\\(SHA-256)};
  \node[box, below=of merkle, fill=green!8]  (sign)    {Ed25519 Signer};
  \node[box, right=0.7cm of dec, fill=red!8]    (quar)    {Quarantine +\\Report};
  ...
  \draw[arrow] (policy) -- (dec);
  \draw[arrow] (dec) -- node[label,right]{yes} (merkle);
  \draw[arrow] (dec) -- node[label,above]{\,no} (quar);
  \draw[arrow] (merkle) -- (sign);
\end{tikzpicture}
\caption{STSS analysis pipeline. The Merkle and signing stages execute
only when policy evaluation yields \textsc{pass} or
\textsc{pass\_with\_warnings}.}
\label{fig:pipeline}
\end{figure}
...
\midrule
FS-001 & HIGH & \texttt{.ssh|.env|/etc/passwd|id\_rsa|.npmrc|.netrc|\textasciitilde/.aws} \\
SH-001 & HIGH & \texttt{subprocess|exec(|eval(|child\_process|execSync|shell=True} \\
NET-001& HIGH / CRIT & \texttt{fetch(|axios|requests.get|urllib|XMLHttpRequest|socket(} \\
SEC-001& HIGH & \texttt{process.env|os.environ} \\
SEC-002& HIGH & Hex$\geq$32 / Base64$\geq$40 chars near key/secret/token/password \\
OBF-001& HIGH & \texttt{base64.b64decode+exec|Buffer.from(...,'base64')+eval} \\
OBF-002& HIGH & \texttt{fromCharCode} chains $\geq$5 \\
OBF-003& HIGH & \texttt{unescape()+eval(} \\
PI-001$\dagger$ & MED & ``ignore previous'', ``you are now operating as'', ``exfiltrate'' \\
CP-001$\dagger$ & MED & ``debug mode'', ``you have been granted'', heading$\to$shell within 3 lines \\
CG-001 & CRIT & Files named \texttt{post\_install.*|setup.sh|install.sh|bootstrap.sh} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Cryptographic operation timing (ms, $n=5$ runs).}
\label{tab:crypto}
\renewcommand{\arraystretch}{1.1}
\scriptsize
\begin{tabular}{lrrrr}
...
Sign (ms)         & 1.15 & 1.57 & 1.66 \\
Verify (ms)$^*$   & 3.54 & 16.32 & 55.44 \\
\bottomrule
\end{tabular}
\\[3pt]
\raggedright\scriptsize $^*$Verify includes full Merkle recomputation from disk.
\end{table}
